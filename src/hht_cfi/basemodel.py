...
class Temperal_Encoder(nn.Module):
    """Sequence encoder: Conv1d → Transformer → LSTM."""
    ...
    def __init__(self, args):
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.hidden_size, nhead=self.args.x_encoder_head,
    ...
    def forward(self, x):
        self.x_dense     = self.conv1d(x).permute(0, 2, 1)
        self.x_dense     = self.mlp1(self.x_dense) + self.x_dense
        self.x_dense_in  = self.transformer_encoder(self.x_dense) + self.x_dense
        output, (hn, cn) = self.lstm(self.x_dense_in)
        self.x_state, cn = hn.squeeze(0), cn.squeeze(0)
        self.x_endoced   = self.mlp(output) + output
...
class Global_interaction(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.hidden_size = self.args.hidden_size
        self.ngate         = MLP_gate(self.hidden_size * 3, self.hidden_size)
    ...
    def forward(self, corr_index, nei_index, nei_num, hidden_state, cn, agent_class):
        ...
        nei_index_t  = nei_index.view(self.N * self.N)
        corr_t       = corr_index.contiguous().view((self.N * self.N, -1))
        if corr_t[nei_index_t > 0].shape[0] == 0:
            return hidden_state, cn
        r_t          = self.relativeLayer(corr_t[nei_index_t > 0])
        inputs_part  = nei_inputs[nei_index_t > 0].float()
        hi_t         = nei_inputs.view((self.N, self.N, self.hidden_size)).permute(1, 0, 2).contiguous().view(-1, self.hidden_size)
        tmp          = torch.cat((r_t, hi_t[nei_index_t > 0], nei_inputs[nei_index_t > 0]), 1)
        nGate        = self.ngate(tmp).float()
        Pos_t        = torch.full((self.N * self.N, 1), 0, device=device).view(-1).float()
        tt           = self.WAr(torch.cat((r_t, hi_t[nei_index_t > 0], nei_inputs[nei_index_t > 0]), 1)).view(-1).float()
        ...
        H_sum        = self.weight(torch.sum(H, 1))
        C            = H_sum + cn
        H            = hidden_state + F.tanh(C)
        return H, C
