...
from .laplace_decoder_joint import Decoder
...
class MyTraj(nn.Module):
    ...
    def forward(self, inputs, edge_pair, epoch, iftest=False):
        batch_abs_gt, batch_norm_gt, batch_split, shift_values, max_values = inputs
        ...
        if self.args.input_offset:
            ...
            offset      = batch_norm_gt[1:self.args.obs_length, :, :] - batch_norm_gt[:self.args.obs_length - 1, :, :]
            position    = batch_norm_gt[:self.args.obs_length, :, :]
            ...
            train_x = batch_norm_gt[:self.args.obs_length, :, :]

        train_x = train_x.permute(1, 2, 0)
        train_y = batch_norm_gt[self.args.obs_length:, :, :].permute(1, 2, 0)
        self.pre_obs = batch_norm_gt[1:self.args.obs_length]

        x_encoded, hidden_state_unsplited, cn = self.Temperal_Encoder.forward(train_x)
        ...
        mdn_out1, mdn_out2 = self.Laplacian_Decoder.forward(
            x_encoded, hidden_state_global, cn_global, shift_values, max_values, batch_split)

        lnnl1, full_pre_tra1 = self.mdn_loss(train_y_gt, mdn_out1)
        lnnl2, full_pre_tra2 = self.mdn_loss(train_y_gt, mdn_out2)
    ...
    def mdn_loss(self, y, y_prime):
        ...
        l2_norm   = torch.norm(out_mu - y, p=2, dim=-1).sum(dim=-1)
        ...
        l2_norm_FDE = torch.norm(out_mu[:, :, -1, :] - y[:, -1, :], p=2, dim=-1)
