...
% - Metric: ADE = single-shot mean MSE over horizon (Eq. 1), positions only.
...
\label{sec:hygiene}
% Two measurement artifacts, each worth 0.2-0.5: (i) noisy single-window val ->
% deterministic multi-window sampler; (ii) hoop nodes deflate the metric (13 vs
% 11 entities) -> landmark-aware metric. After fixes, val tracks Kaggle.

\subsection{Main comparison}
\label{sec:main-results}
% One table (breadth + winners). Numbers: baseline ~3.7; STGCNN ~3.8;
% CVAE/SocialVAE 4.3/5.0; MART-300ep 3.79; EqMotion single 3.33 (Kaggle 3.2) /
% ensemble 3.25; aug-MART 5k 3.11 (Kaggle 3.01); +D2 TTA 3.08;
% PRODUCTION blend 3.07 / Kaggle 2.98.

\subsection{Ablations on the winners}
\label{sec:ablations}
% Focused on EqMotion/aug-MART: isotropic vs aniso norm; hoop nodes (and "more
% landmarks all hurt"); O(2) augmentation; epochs/capacity (bigger overfits or
% mis-aligns min_ade); D2 TTA; ensembling/blend.

\subsection{The ball is the bottleneck (critical analysis)}
\label{sec:ball}
% CROWN JEWEL -- the 25% interpretation marks live here. Error decomposes into
% players ~2.07 (floor) + ball ~13.5 (6.4x harder, ~the whole 3.1->2.6 gap).
% The ball floor is 7-WAY confirmed: capacity, aggregation, head-selection (x2
% models), possession, physics, residual, cross-arch (rho=0.93), and direct
% mean_mse regression (overfits to 3.20). HONEST verdict: even point-estimate
% generative models submit the conditional mean, so the generative lever is
% CLOSED too. The 3.1->2.6 gap is irreducible ball multimodality undetermined by
% an 8-frame past -- NOT an architecture/optimization artifact.
