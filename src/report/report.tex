...
\begin{abstract}
% 1 paragraph: task (NBA C=8 -> H=12, single-shot mean MSE); we sweep the
...
\begin{equation}
\label{eq:mse}
\mathrm{MSE} = \frac{1}{H\times N}\sum_{t=C+1}^{C+H}\sum_{n}
\big\lVert \hat{z}^n_t - z^n_t \big\rVert_2^2,
\qquad \hat{z}^n_t, z^n_t \in \mathbb{R}^2 .
\end{equation}
% NB: the proposal/Kaggle call this "ADE", but the formula is mean SQUARED error
% (a squared L2, not a displacement). We refer to it as MSE for accuracy.
...
All numbers are \texttt{val/mse\_ft}: single-shot mean squared error (feet$^2$,
$H{=}12$) on the 11 real entities of the held-out fold (497 sequences), matching
the Kaggle metric (Eq.~\ref{eq:mse}; the proposal labels it ``ADE'' but it is an
MSE). Two fixes make the fold a reliable selector: a deterministic multi-window
sampler in place of a noisy single-window draw, and a metric that excludes the
static hoop nodes (Sec.~\ref{sec:eqmotion}) from the average. The
val$\rightarrow$Kaggle gap is then consistent ($\sim{+}0.08$, val slightly
pessimistic).

\subsection{Main comparison}
\label{sec:main-results}
Table~\ref{tab:main} collects the families we explored. Two patterns stand out.
First, the strong point predictors (STGCNN, EqMotion, MART) beat the non-graph
transformer baseline, but graph structure alone is not enough: the
learned-dynamic-graph dNRI ($\sim$5.5) barely edges the baseline, and the
multimodal VAEs (STGCNN-CVAE, SocialVAE) trail the point predictors. The metric
rewards the conditional mean, and MART -- though trained with a best-of-$K$
(minADE) loss like the VAEs -- recovers that mean by averaging its $K$ diverse
heads (mean-of-$K$), whereas the sampled VAE outputs estimate it less well.
...
STGCNN, EqMotion and MART are Optuna-tuned (Appendix~\ref{sec:hp}); the STGCNN
entry is its tuned top-5 retrain + TTA ensemble. The production blend combines
the two best models (Sec.~\ref{sec:tradeoff}).}
\label{tab:main}
\begin{center}
\begin{tabular}{llcc}
\toprule
...
\midrule
Non-graph        & Transformer + velocity            & $\sim$5.8 & 5.79 \\
Fixed graph      & STGCNN (tuned, TTA ensemble)      & $\sim$3.8 & -- \\
Dynamic graph    & dNRI (learned, single)            & $\sim$5.5 & 5.48 \\
Dynamic graph    & dNRI (learned, ensemble)          & $\sim$5.3 & 5.22 \\
...
\label{sec:ablations}
All three models in this section have their
architecture and optimization hyperparameters chosen by Optuna (TPE) studies
rather than by hand; full search spaces, selection metrics and winning
configurations are in Appendix~\ref{sec:hp}.
...
(Table~\ref{tab:abl}, left). These deltas are on top of an Optuna-tuned EqMotion
configuration, and the top trials were statistically tied, so the structural
levers (normalization, hoops), not the continuous hyperparameters, dominate. Hoops are the \emph{right amount} of court
...
$3.11$ is a model/data floor rather than a tuning artifact: budget-matched SGDR
warm restarts tie it ($3.116$), and a 10k-epoch single-cosine run does not beat
it. The remaining free gains come from variance reduction orthogonal to the
...
levers. \textbf{Right:} augmented MART scaling -- augmentation is what lets the
larger model and longer schedule pay off.}
\label{tab:abl}
\begin{center}
...
regression -- the ball MSE never leaves the $13$--$14$~ft$^2$ band. Direct
\texttt{mean\_mse} regression is the clearest case (Fig.~\ref{fig:loss}):
optimising the single-shot target overfits (best $3.20$, then climbing to $3.58$)
and loses to $3.11$, so MART's head-averaged mean is the better-regularized
estimate of the conditional mean. This also bounds generative models: any
point-estimate model (MDN, flow-matching, diffusion) submits the conditional mean
under a single-shot metric, and that target cannot beat $3.11$. Published
MoFlow/SocialVAE NBA results report best-of-$K$ (minADE) over many samples -- a
protocol our single-submission setup cannot use. The residual error is thus irreducible ball
...
\end{center}
\caption{One play (context solid, future dashed, dot $=$ position at $t{=}C$).
Players follow paths the model recovers tightly; the ball's $K{=}20$ modes
(orange) fan toward distinct futures, only one realized (black). Their mean is far
from each -- the source of the ball error.}
\label{fig:play}
\end{figure}
...
\label{sec:hp}
The three tuned models (STGCNN, EqMotion, MART) each use an Optuna study with a
TPE sampler and SQLite-backed persistence; the per-model search spaces, budgets
and selection metrics are in Table~\ref{tab:hp}. Two protocol details matter for
interpreting the results. (i)~\textbf{STGCNN} is tuned directly on the Kaggle
metric \texttt{val/mse\_ft} with median pruning (60 trials, 50 epochs each); the
top-5 trials are then retrained at 250 epochs and combined with court-symmetry
test-time augmentation into the single ensemble reported in
Table~\ref{tab:main}. (ii)~\textbf{MART}'s search is ranked by the \emph{last}
epoch's mean-of-$K$ error rather than the best epoch (to avoid double-dipping the
validation signal) with no pruner, followed by a 100-epoch retrain of the
winning configuration. \textbf{EqMotion} is pruned on \texttt{val/loss} with a
median pruner (40 trials, 50 epochs); its top trials were statistically tied
(val/loss $0.0134$--$0.0135$), confirming the model is insensitive to these
knobs within range and that the structural levers in Sec.~\ref{sec:ablations}
are what move the metric. The augmented 7.5M MART backbone
(Sec.~\ref{sec:augmart}) is a deliberately hand-scaled configuration, not an
Optuna output.

\begin{table}[h]
\caption{Optuna search spaces. ``cat'' = categorical choice set, ``log'' =
log-uniform continuous range.}
\label{tab:hp}
\begin{center}
\small
\begin{tabular}{lll}
\toprule
\textbf{Model} & \textbf{Searched hyperparameter} & \textbf{Range / choices} \\
\midrule
\multirow{8}{*}{EqMotion}
 & hidden\_nf            & cat $\{32, 64, 128, 256\}$ \\
 & hid\_channel         & cat $\{8, 16, 32, 64\}$ \\
 & n\_layers            & int $[2, 5]$ \\
 & learning rate        & log $[10^{-4}, 10^{-2}]$ \\
 & weight decay         & log $[10^{-6}, 10^{-2}]$ \\
 & batch size           & cat $\{32, 64, 128\}$ \\
 & gradient clip        & $[0.25, 2.0]$ \\
\cmidrule(l){2-3}
 & \emph{selection / budget} & val/loss, median pruning, 40 trials $\times$ 50 ep \\
\midrule
\multirow{8}{*}{MART}
 & learning rate        & log $[10^{-4}, 10^{-3}]$ \\
 & weight decay         & log $[10^{-10}, 10^{-4}]$ \\
 & dropout              & cat $\{0.0, 0.1, 0.2\}$ \\
 & num\_layers          & cat $\{2, 3, 4\}$ \\
 & model\_dim           & cat $\{32, 64, 128\}$ \\
 & hidden\_dim          & cat $\{64, 128, 256\}$ \\
 & decoder\_hidden\_dim & cat $\{64, 128, 256\}$ \\
\cmidrule(l){2-3}
 & \emph{selection / budget} & last-epoch mean-$K$ MSE, no pruner, 40 $\times$ 80 ep, retrain 100 ep \\
\midrule
\multirow{11}{*}{STGCNN}
 & graph\_space         & cat $\{$pos, vel, both$\}$ \\
 & decoder              & cat $\{$autoreg, txp$\}$ \\
 & n\_stgcnn            & int $[1, 3]$ \\
 & hidden\_feat         & cat $\{32, 64, 128\}$ \\
 & kernel\_size         & cat $\{3, 5\}$ \\
 & n\_txpcnn            & cat $\{3, 5, 7\}$ \\
 & learning rate        & log $[10^{-4}, 3\!\times\!10^{-3}]$ \\
 & weight decay         & log $[10^{-7}, 10^{-3}]$ \\
 & batch size           & cat $\{64, 128, 256\}$ \\
 & augmentation         & cat $\{$on, off$\}$ \\
\cmidrule(l){2-3}
 & \emph{selection / budget} & val/mse\_ft, median pruning, 60 $\times$ 50 ep, top-5 retrain @250 ep + TTA \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
