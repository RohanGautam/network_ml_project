"""Retrain the top-K dNRI Optuna trials at full epoch budget and emit two Kaggle
submissions: (1) the single best model, (2) the top-K mean ensemble.

Mirrors src/equivariance/retrain_topk.py. The Optuna study at --study-path is
opened read-only; trials are filtered to COMPLETE state and ranked by val/loss.
Each rank is retrained from scratch with its trial params (no warm-start),
checkpointed on best val/loss, then used to predict the test set. Predictions
are stacked across ranks; rank-0 alone yields the "best" CSV and the
per-coordinate mean yields the "ensemble" CSV.
"""
...
from dynamic.dnri_nba import (  # noqa: E402
    SUBMISSION_DIR,
    TEST_DIR,
    NBADNRILightningModel,
)
...
def train_one(trial, rank, args, ckpt_dir):
    ...
    # MLP/prior head sizes auto-scale off the RNN hidden — must match tune_dnri.py
    encoder_mlp_hidden = max(32, p["encoder_rnn_hidden"])
    prior_hidden_size = max(32, p["encoder_rnn_hidden"])

    model = NBADNRILightningModel(
        context_size=8,
        horizon_size=12,
        num_edge_types=p["num_edge_types"],
        lr=p["lr"],
        weight_decay=p["weight_decay"],
        encoder_hidden=p["encoder_hidden"],
        encoder_rnn_hidden=p["encoder_rnn_hidden"],
        ...
        decoder_dropout=p["decoder_dropout"],
        gumbel_temp=p["gumbel_temp"],
        kl_coef=p["kl_coef"],
    )
    ...
    best = NBADNRILightningModel.load_from_checkpoint(
...
def main():
    ...
    parser.add_argument("--run-name", default="dnri_topk_retrain")
    ...
    ckpt_dir = PROJECT_ROOT / "models" / "dnri_topk"
    ...
    best_path = SUBMISSION_DIR / f"solution_dnri_best_{timestamp}.csv"
    write_submission(common_ids, stacked[0], best_path)

    ensemble_path = SUBMISSION_DIR / f"solution_dnri_ensemble_{timestamp}.csv"
