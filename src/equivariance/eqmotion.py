...
class Feature_learning_layer(nn.Module):
    ...
    def __init__(
        ...
        self.node_att_mlp = nn.Sequential(
            nn.Linear(hidden_c, hidden_c), act_fn, nn.Linear(hidden_c, 1)
        )
    ...
    def node_model(self, x, edge_feat):
        ...
        mask = (torch.ones((agent_num, agent_num)) - torch.eye(agent_num)).type_as(
            edge_feat
        )
    ...
    def non_linear(self, coord):
        ...
        product = torch.matmul(q.unsqueeze(-2), k.unsqueeze(-1)).squeeze(
            -1
        )  # (B,N,C,1)
    ...
    def forward(self, h, coord, vel, edge_attr=None, node_attr=None, category=None):
        ...
        if self.input_reasoning:
            coord = self.aggregate_coord_reasoning(
                coord, edge_feat, coord_diff, category, h
            )
        else:
...
class EqMotion(nn.Module):
    ...
    def transform_edge_attr(self, edge_attr):
        ...
        interaction_category = F.one_hot(
            edge_attr.long(), num_classes=self.category_num
        )
    ...
    def calc_category(self, h, coord):
        ...
        mask = mask[None, :, :, None].repeat(batch_size, 1, 1, 1)
        node_new = self.node_mlp(
            torch.cat([h, torch.sum(mask * edge_feat, dim=2)], dim=-1)
        )
        ...
        interaction_category = F.softmax(
            self.category_mlp(edge_feat_input_new) / self.tao, dim=-1
        )
    ...
    def forward(self, h, x, vel, edge_attr=None):
        ...
        for i in range(0, self.n_layers):
            h, x, _ = self._modules["gcl_%d" % i](
                h, x, vel, edge_attr=edge_attr, category=category
            )
